...
public:
    explicit ArenaAllocator(const size_t max_num_bytes)
        ...
        , m_buffer { new byte[max_num_bytes] }
        , m_offset { m_buffer }
    {
    }
    ...
    ArenaAllocator(ArenaAllocator&& other) noexcept
        ...
        , m_offset { exchange(other.m_offset, nullptr) }
    {
    }
    ...
    [[nodiscard]] T* alloc()
    {
        ...
        const auto aligned_address = align(alignof(T), sizeof(T), pointer, remaining_num_bytes);
        if (aligned_address == nullptr) {
            throw bad_alloc {};
        }
        m_offset = static_cast<byte*>(aligned_address) + sizeof(T);
    ...
    [[nodiscard]] T* emplace(Args&&... args)
    {
        ...
        return new (allocated_memory) T { forward<Args>(args)... };
    }
